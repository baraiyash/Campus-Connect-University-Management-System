...
public class TeacherLeaveDetails extends JFrame implements ActionListener {
    ...
    private void fetchLeaveDetails(String leaveId) {
        try {
            Conn c = new Conn();
            String sql = "SELECT * FROM teacherleave WHERE id = ?";
            ...
            if (rs.next()) {
                leaveDetails.append("Leave ID - ").append(rs.getInt("id")).append("\n")
                            .append("Employee ID - ").append(rs.getInt("empId")).append("\n")
    ...
    private void updateLeaveStatus(String status) {
        ...
        try {
            Conn c = new Conn();
            String sql = "UPDATE teacherleave SET status = ? WHERE id = ?";
            PreparedStatement pstmt = c.c.prepareStatement(sql);
    ...
    public static void main(String[] args) {
        new TeacherLeaveDetails();  // Corrected class instantiation
    }
}
